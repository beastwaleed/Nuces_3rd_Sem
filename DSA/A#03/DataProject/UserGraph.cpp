...
void UserProfile::addNotification(string notification)
{
    notifications.enqueue(notification);
}

void UserProfile::receiveMessage(string sender, string message)
{
    messages.enqueue("From " + sender + ": " + message);
    addNotification("New message received from " + sender);
}

void UserProfile::viewNotifications()
{
    ...
    {
        Node* temp = notifications.getFront();
        cout << "- " << temp->data << " ";
        temp->obj.display();
...
void UserProfile::addPost(string postContent)
{
    posts->push(postContent);
    cout << "Post added successfully!" << endl;
}

void UserProfile::showPosts()
{
    if (posts->isEmpty())
    {
    ...
    while (!posts->isEmpty())
    {
        string postContent = posts->peek();
        posts->getTopDateTime().display();
        cout << "- " << postContent << endl;
        tempStack.push(posts->pop());
    }

    while (!tempStack.isEmpty())
    {
        posts->push(tempStack.pop());
    }
}

void UserProfile::addFollower(string followerName)
{
...
void UserProfile::addFriend(string friendName)
{
...
void UserProfile::displayFriends()
{
...
UserProfile* UserGraph::findUser(UserProfile* root, string username)
{
...
void UserGraph::signup(string username, string password, string city)
{
...
bool UserGraph::login(string username, string password)
{
    if (currentUser)
    {
...
void UserGraph::sendFollowRequest(string sender, string receiver)
{
...
void UserGraph::acceptFollowRequest(string username)
{
...
void UserGraph::addFriend(string user1, string user2)
{
...
void UserGraph::displayUserFollowers(string username)
{
    UserProfile* user = findUser(head, username);
    if (user)
    {
...
void UserGraph::displayUserFriends(string username)
{
    UserProfile* user = findUser(head, username);
    if (user)
    {
...
void UserGraph::addPost(string postContent)
{
    if (!currentUser)
    {
        cout << "No user is currently logged in." << endl;
...
void UserGraph::sendMessage(string receiver, string message)
{
    if (!currentUser)
    {
...
void UserGraph::viewNotifications()
{
    if (!currentUser)
    {
...
void UserGraph::blockFriend(string user1, string user2)
{
    if (!currentUser || currentUser->name != user1)
    {
...
void UserGraph::unblockFriend(string user1, string user2)
{
    if (!currentUser || currentUser->name != user1)
    {
